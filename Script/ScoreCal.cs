...
public class ScoreCal : MonoBehaviour
{
    ...
    {
        m_nInit = 0;
        m_nValue = m_pMNInfoScript.C_LoadNInfo(MultipleInfo.E_NINFO.E_GOLD) * GOLDSCORE;

        m_nGoldScore = m_nValue;

        m_pMNInfoScript.C_UpdateNInfo(m_nGoldScore, MultipleInfo.E_NINFO.E_GOLDSCORE);

        m_pCounterScript.C_NumberCount(m_nInit, m_nValue,
            ref m_nResult );

        m_pMNInfoScript.C_UpdateNInfo(m_nResult, MultipleInfo.E_NINFO.E_SCORE);
